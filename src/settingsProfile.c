...
inline void spUpdate(settingsProfile *profile, int mouseRawX, int mouseRawY, float *mouseX, float *mouseY){
	...
	if(profile->enhancePointerPrecision && profile->acceleration == 1){
		...
		// Calculate accelerated mouse deltas
		float mouseXplusRemainder = mouseRawX * profile->pixelGain;
		float mouseYplusRemainder = mouseRawY * profile->pixelGain;
		...
		if(profile->windowsVersion == 2){
			...
			if(mouseXplusRemainder >= 0.f){
				*mouseX = mouseXplusRemainder;
			}else{
				*mouseX = -mouseXplusRemainder;
			}
			profile->previousMouseXRemainder = 0;//mouseXplusRemainder - *mouseX;
